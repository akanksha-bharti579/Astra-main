...
export interface cost_breakdown {
  ...
  output_cost: number;
  total_cost: number;
  input_carbon: number; // gCO2e
  output_carbon: number; // gCO2e
  total_carbon: number; // gCO2e
}
...
export interface pricing_info {
  ...
  carbon?: {
    input: number; // gCO2e per 1k tokens
    output: number; // gCO2e per 1k tokens
  };
}
