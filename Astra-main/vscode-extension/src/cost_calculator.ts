 ...
 * @param model - model name
 * @param tokens - token count
 * @returns carbon emission in gCO2e
 */
export function calculate_carbon(model: string, tokens: number): number {
  const matchedKey = findMatchingKey(model);

  if (!matchedKey) {
    return (tokens / 1000) * 0.2; // Default fallback (small model equivalent)
  }

  const model_pricing = pricing[matchedKey];
  // If carbon data is missing, fallback to 0.5g/1k (conservative average)
  const carbon_rate = model_pricing.carbon ? model_pricing.carbon.input : 0.5;
  return (tokens / 1000) * carbon_rate;
}

/**
 * get detailed cost and carbon breakdown
 * @param model - model name
 * @param input_tokens - input token count
 * @param output_tokens - output token count (optional)
 * @returns detailed cost and carbon breakdown
 */
...
export function get_cost_breakdown(
  ...
  // Calculate Carbon
  const carbon_input_rate = model_pricing.carbon ? model_pricing.carbon.input : 0.5;
  const carbon_output_rate = model_pricing.carbon ? model_pricing.carbon.output : 1.5;

  const input_carbon = (input_tokens / 1000) * carbon_input_rate;
  const output_carbon = (output_tokens / 1000) * carbon_output_rate;
  ...
  return {
    ...
    input_carbon,
    output_carbon,
    total_carbon: input_carbon + output_carbon
  };
}
...
/**
 * Calculate the total carbon (input + output) for a model given token counts
 */
export function calculate_total_carbon(model: string, input_tokens: number, output_tokens: number = 0): number {
  const breakdown = get_cost_breakdown(model, input_tokens, output_tokens);
  return breakdown.total_carbon;
}
