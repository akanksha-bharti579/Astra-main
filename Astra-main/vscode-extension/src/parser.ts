...
import { estimate_tokens, calculate_cost, calculate_carbon } from './cost_calculator';
...
/**
 * Explicitly load the graph from cache (disk)
 */
export async function loadCache(workspaceRoot: string): Promise<CodespaceGraph | null> {
  const graph = await loadIndex(workspaceRoot);
  if (graph) {
    cachedGraph = graph;
  }
  return graph;
...
export async function indexWorkspace(rootPath: string): Promise<CodespaceGraph> {
  ...
  try {
    ...
    if (newUnitsToClassify.length > 0) {
      ...
      // Use quick detection if no API key is present
      const useQuickDetection = !process.env.GEMINI_API_KEY;
      if (!useQuickDetection) {
        console.log('✨ Using Gemini AI for advanced classification');
      }
      const classifications = await batchClassifyApis(bundles, useQuickDetection);
...
export function extractModelFromCode(code: string, provider: string): string {
  // Look for model parameter with various styles
  // 1. model="gpt-4" or model='gpt-4'
  let modelMatch = code.match(/model\s*[:=]\s*["']([^"']+)["']/);
  if (modelMatch) return modelMatch[1];

  // 2. model_name="gemini-2.0"
  modelMatch = code.match(/model_name\s*[:=]\s*["']([^"']+)["']/);
  if (modelMatch) return modelMatch[1];

  // 3. GenerativeModel("gemini-1.5-pro") - common in Python Google SDK
  modelMatch = code.match(/GenerativeModel\s*\(\s*["']([^"']+)["']\s*\)/);
  if (modelMatch) return modelMatch[1];

  // 4. OpenAI(model="...")
  modelMatch = code.match(/model\s*=\s*["']([^"']+)["']/);
  if (modelMatch) return modelMatch[1];
