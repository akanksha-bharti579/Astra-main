...
export class cost_tree_provider implements vscode.TreeDataProvider<cost_tree_item> {
  ...
  private get_root_items(): cost_tree_item[] {
    ...
    const total_carbon = this.use_mock_data
      ? calls.reduce((sum, call) => sum + (call.estimated_carbon || 0), 0)
      : this.total_carbon_cache;

    const call_count = calls.length;

    // summary section - PRIMARY: Carbon, SECONDARY: Cost
    items.push(new cost_tree_item(
      `🌱 Footprint: ${total_carbon.toFixed(2)} gCO2e`,
  ...
  private get_call_items(): cost_tree_item[] {
    ...
    // Sort by Carbon (High -> Low)
    calls.sort((a, b) => (b.estimated_carbon || 0) - (a.estimated_carbon || 0));

    return calls.map(call => {
      // Format: Model • Carbon • Cost
      const carbon = call.estimated_carbon ? `${call.estimated_carbon.toFixed(2)}g` : '0g';
      const label = `${call.model} • ${carbon} • $${call.estimated_cost.toFixed(4)}`;
  ...
  private get_optimization_items(): cost_tree_item[] {
    if (this.suggestions.length === 0) {
      return [new cost_tree_item('no green improvements found', vscode.TreeItemCollapsibleState.None, 'optimization_item')];
    }
  ...
  private get_simulator_items(): cost_tree_item[] {
    const items: cost_tree_item[] = [];
    const calls = this.use_mock_data ? this.get_mock_data() : this.detected_calls;
    const total_carbon = calls.reduce((sum, call) => sum + (call.estimated_carbon || 0), 0);
    ...
    // projections
    const daily_carbon = total_carbon * this.user_count;
    const monthly_carbon = daily_carbon * 30; // gCO2e
    const yearly_carbon_kg = (daily_carbon * 365) / 1000; // kgCO2e

    items.push(new cost_tree_item(
      `daily: ${daily_carbon.toFixed(1)} gCO2e`,
      vscode.TreeItemCollapsibleState.None,
      'simulator_item'
    ));

    items.push(new cost_tree_item(
      `monthly: ${(monthly_carbon / 1000).toFixed(2)} kgCO2e`,
      vscode.TreeItemCollapsibleState.None,
      'simulator_item'
    ));

    items.push(new cost_tree_item(
      `yearly: ${yearly_carbon_kg.toFixed(2)} kgCO2e`,
      vscode.TreeItemCollapsibleState.None,
      'simulator_item'
    ));

    // Climate Budget Clock / Equivalent
    // 1 Tree absorbs ~21kg CO2 per year. 
    // How many trees needed to offset yearly footprint?
    const trees_needed = Math.ceil(yearly_carbon_kg / 21);

    let impactText = '';
    let impactIcon = '';

    if (yearly_carbon_kg < 1) {
      impactText = 'Net Zero Compatible 🌿';
      impactIcon = 'check';
    } else if (trees_needed <= 5) {
      impactText = `Offset: Plant ${trees_needed} trees 🌳`;
      impactIcon = 'heart';
    } else {
      impactText = `Offset: Plant ${trees_needed} trees 🌲`;
      impactIcon = 'warning';
  ...
  private get_project_items(): cost_tree_item[] {
    ...
    for (const file of this.top_expensive_files) {
      items.push(new cost_tree_item(
        `${path.basename(file.path)} (${file.carbon.toFixed(2)}g CO2e)`,
        vscode.TreeItemCollapsibleState.None,
        'file_item',
        undefined,
        file.path
    ...
    return [
      {
        ...
        estimated_carbon: 4.0 // High carbon
      },
      {
        line: 12,
        provider: 'anthropic',
  ...
  // Cached display data
  private top_expensive_files: { path: string; cost: number; carbon: number }[] = [];
  private total_cost_cache: number = 0;
  private total_carbon_cache: number = 0;
  ...
  private recalculate_stats() {
    ...
    this.total_carbon_cache = this.detected_calls.reduce((sum, call) => sum + (call.estimated_carbon || 0), 0);

    // 2. Top Files (previously in get_project_items)
    if (!this.project_graph) return;

    const fileCosts = new Map<string, { cost: number, carbon: number }>();
    ...
    for (const call of this.detected_calls) {
      if (call.file_path) {
        const current = fileCosts.get(call.file_path) || { cost: 0, carbon: 0 };
        fileCosts.set(call.file_path, {
          cost: current.cost + call.estimated_cost,
          carbon: current.carbon + (call.estimated_carbon || 0)
        });
      }
    }

    this.top_expensive_files = Array.from(fileCosts.entries())
      .sort(([, statsA], [, statsB]) => statsB.carbon - statsA.carbon) // Sort by Carbon
      .slice(0, 5) // Top 5
      .map(([path, stats]) => ({ path, cost: stats.cost, carbon: stats.carbon }));
